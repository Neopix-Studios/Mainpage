import { useState, useEffect } from 'react';
import { motion, AnimatePresence } from 'motion/react';
import { ArrowLeft, ArrowUpRight, Play, MonitorPlay } from 'lucide-react';
import { GallerySlider } from '../components/GallerySlider';

const mixedImages = [
  "https://images.unsplash.com/photo-1542751371-adc38448a05e?auto=format&fit=crop&q=80",
  "https://images.unsplash.com/photo-1511512578047-dfb367046420?auto=format&fit=crop&q=80",
  "https://images.unsplash.com/photo-1552820728-8b83bb6b773f?auto=format&fit=crop&q=80",
  "https://images.unsplash.com/photo-1531297122539-5692f6e14c41?auto=format&fit=crop&q=80",
  "https://images.unsplash.com/photo-1550745165-9bc0b252726f?auto=format&fit=crop&q=80",
  "https://images.unsplash.com/photo-1605810230434-7631ac76ec81?auto=format&fit=crop&q=80",
];

const getVideoGridClass = (index: number) => {
  const mdPattern = [
    "lg:col-span-8 lg:row-span-2 md:col-span-12 md:row-span-2",
    "lg:col-span-4 lg:row-span-1 md:col-span-6 md:row-span-1",
    "lg:col-span-4 lg:row-span-1 md:col-span-6 md:row-span-1",
    "lg:col-span-4 lg:row-span-1 md:col-span-6 md:row-span-1",
    "lg:col-span-4 lg:row-span-1 md:col-span-6 md:row-span-1",
    "lg:col-span-4 lg:row-span-1 md:col-span-6 md:row-span-1",
    "lg:col-span-12 lg:row-span-2 md:col-span-12 md:row-span-2"
  ];
  return `col-span-12 ${mdPattern[index % 7]}`;
};

const gameVideos = [
  {
    id: "dignitized",
    title: "Project Dignitized",
    subtitle: "Cinematic & Gameplay",
    cover: mixedImages[5],
    color: "#ff6a00",
    videos: [
      { title: "Story Reveal Trailer", duration: "02:45", thumb: mixedImages[5], views: "2.4M", type: "Cinematic" },
      { title: "Cryo Island 4K Gameplay Part 1", duration: "18:20", thumb: mixedImages[4], views: "890K", type: "Gameplay" },
      { title: "Developer Deep Dive Ep. 1", duration: "45:00", thumb: mixedImages[1], views: "340K", type: "Devlog" },
      { title: "The Cronimals Identity", duration: "01:30", thumb: mixedImages[3], views: "1.2M", type: "Teaser" },
      { title: "Soundtrack Recording BTS", duration: "08:15", thumb: mixedImages[0], views: "120K", type: "BTS" },
      { title: "Multiplayer Tech Test", duration: "12:40", thumb: mixedImages[2], views: "550K", type: "Showcase" },
      { title: "E3 Stage Presentation", duration: "24:00", thumb: mixedImages[5], views: "4.1M", type: "Event" }
    ]
  },
  {
    id: "cryora-records",
    title: "Cryora Records",
    subtitle: "Original Soundtracks & Live",
    cover: mixedImages[1],
    color: "#a3a3a3",
    videos: [
      { title: "Symphony of the Fallen", duration: "05:12", thumb: mixedImages[4], views: "1.1M", type: "Music Video" },
      { title: "Studio Sessions: Percussion", duration: "10:30", thumb: mixedImages[0], views: "45K", type: "BTS" },
      { title: "Live at Royal Albert Hall", duration: "1:20:00", thumb: mixedImages[1], views: "890K", type: "Live" },
      { title: "Electronic Synthesis Breakdown", duration: "15:45", thumb: mixedImages[2], views: "115K", type: "Devlog" },
      { title: "Vinyl Mastering Process", duration: "06:20", thumb: mixedImages[3], views: "80K", type: "Documentary" },
      { title: "Theme Suite Medley", duration: "12:00", thumb: mixedImages[5], views: "420K", type: "Music" },
      { title: "Artist Interview: K. Yamada", duration: "22:15", thumb: mixedImages[4], views: "65K", type: "Interview" }
    ]
  },
  {
    id: "project-one",
    title: "Project One Online",
    subtitle: "Updates & Community",
    cover: mixedImages[0],
    color: "#3b82f6",
    videos: [
      { title: "Global Launch Trailer", duration: "02:10", thumb: mixedImages[0], views: "5.5M", type: "Cinematic" },
      { title: "Update 1.4: The Abyss", duration: "04:30", thumb: mixedImages[3], views: "1.8M", type: "Update" },
      { title: "World First Boss Kill Reaction", duration: "08:45", thumb: mixedImages[5], views: "900K", type: "Community" },
      { title: "Beginner's Guide to Crafting", duration: "14:20", thumb: mixedImages[1], views: "2.1M", type: "Guide" },
      { title: "PvP Tournament 2025 Finals", duration: "48:00", thumb: mixedImages[4], views: "3.2M", type: "Esports" },
      { title: "Year One Retrospective", duration: "18:30", thumb: mixedImages[2], views: "750K", type: "Documentary" },
      { title: "Next Expansion Teaser", duration: "00:45", thumb: mixedImages[0], views: "4.8M", type: "Teaser" }
    ]
  }
];

export const VideosPage = () => {
  const [selectedGame, setSelectedGame] = useState<typeof gameVideos[0] | null>(null);
  const [hoveredVideo, setHoveredVideo] = useState<number | null>(null);

  useEffect(() => {
    window.scrollTo({ top: 0, behavior: 'smooth' });
  }, [selectedGame]);

  return (
    <div className="min-h-screen bg-[#030303] pb-32 relative z-10 w-full overflow-hidden">
      <AnimatePresence mode="wait">
        {!selectedGame ? (
          <motion.div 
            key="list" 
            initial={{ opacity: 0 }} 
            animate={{ opacity: 1 }} 
            exit={{ opacity: 0, y: -20 }}
            transition={{ duration: 0.5 }}
          >
            {/* Gallery Hero Slider */}
            <GallerySlider images={mixedImages} isVideo={true}>
              <div className="px-6 md:px-12 max-w-[1600px] mx-auto w-full">
                <motion.div 
                  initial={{ opacity: 0, y: 20 }}
                  animate={{ opacity: 1, y: 0 }}
                  transition={{ duration: 0.6 }}
                  className="flex flex-col items-center text-center mt-12 md:mt-24 pointer-events-none"
                >
                  <div className="flex items-center gap-4 mb-4">
                     <span className="text-xs font-bold tracking-[0.3em] uppercase text-white/50 border border-white/20 px-3 py-1 bg-black/20 backdrop-blur-md">Broadcast</span>
                  </div>
                  <h1 className="text-5xl md:text-8xl font-black tracking-tighter text-white uppercase drop-shadow-2xl">
                    Cinematic <span className="text-transparent bg-clip-text bg-gradient-to-r from-white to-[#888]">Vault</span>
                  </h1>
                </motion.div>
              </div>
            </GallerySlider>

            {/* Project Selection Folders */}
            <div className="px-4 md:px-8 xl:px-12 max-w-[1600px] mx-auto w-full mt-24">
              <div className="flex flex-col md:flex-row md:items-end justify-between border-b border-white/10 pb-6 mb-12">
                <div>
                  <h2 className="text-2xl md:text-4xl font-black text-white uppercase tracking-tighter mb-2">
                    Browse Series
                  </h2>
                  <p className="text-[#888] text-sm uppercase tracking-widest font-bold">Trailers, Devlogs & Showcases</p>
                </div>
                <div className="mt-4 md:mt-0 opacity-50">
                   <MonitorPlay className="w-8 h-8" />
                </div>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-10">
                {gameVideos.map((game, index) => (
                  <motion.div 
                    key={game.id}
                    initial={{ opacity: 0, scale: 0.95, y: 20 }}
                    whileInView={{ opacity: 1, scale: 1, y: 0 }}
                    viewport={{ once: true }}
                    transition={{ duration: 0.6, delay: 0.1 * index }}
                    onClick={() => setSelectedGame(game)}
                    className="group cursor-pointer relative aspect-[16/10] bg-[#050505] border border-white/10 overflow-hidden"
                  >
                     <div className="absolute inset-0 z-0">
                       <img 
                         src={game.cover} 
                         alt={game.title} 
                         className="w-full h-full object-cover transition-transform duration-[2s] group-hover:scale-105 opacity-40 group-hover:opacity-70 filter grayscale group-hover:grayscale-0"
                       />
                     </div>
                     <div className="absolute inset-0 bg-gradient-to-t from-black via-black/40 to-transparent z-10" />
                     
                     {/* Play Button Decoration */}
                     <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none z-20">
                       <div className="w-16 h-16 rounded-full border border-white/20 bg-black/40 backdrop-blur-md flex items-center justify-center scale-75 group-hover:scale-100 transition-transform duration-500">
                          <Play className="w-6 h-6 text-white ml-1" fill="currentColor" />
                       </div>
                     </div>

                     <div className="absolute inset-0 p-8 flex flex-col justify-between z-30">
                        <div className="flex items-start justify-between">
                            <span className="px-3 py-1 border border-white/20 text-white/70 text-[10px] font-bold uppercase tracking-widest backdrop-blur-md bg-black/40">
                              {game.videos.length} Episodes
                            </span>
                            <div className="w-10 h-10 rounded-full border border-white/20 bg-black/40 backdrop-blur-md flex items-center justify-center -translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-500">
                               <ArrowUpRight className="w-4 h-4 text-white" />
                            </div>
                        </div>

                        <div className="transform transition-transform duration-500 group-hover:-translate-y-2">
                           <h3 className="text-3xl font-black text-white uppercase tracking-tighter mb-1 transition-colors duration-500" style={{ color: game.color }}>
                              {game.title}
                           </h3>
                           <p className="text-white/50 text-[10px] font-bold tracking-[0.2em] uppercase">
                              {game.subtitle}
                           </p>
                        </div>
                     </div>
                  </motion.div>
                ))}
              </div>
            </div>
          </motion.div>
        ) : (
          <motion.div 
            key="gallery" 
            initial={{ opacity: 0, y: 30 }} 
            animate={{ opacity: 1, y: 0 }} 
            exit={{ opacity: 0, scale: 0.95 }}
            transition={{ duration: 0.6 }}
            className="pt-24"
          >
            {/* Specific Project Detail Header */}
            <div className="px-4 md:px-8 xl:px-12 max-w-[1600px] mx-auto w-full mb-16">
              <button 
                onClick={() => setSelectedGame(null)}
                className="group flex items-center text-[#888] hover:text-white transition-colors text-xs font-bold uppercase tracking-[0.2em] mb-12"
              >
                <ArrowLeft className="w-4 h-4 mr-3 transform group-hover:-translate-x-2 transition-transform" />
                Return to Series
              </button>
              
              <div className="flex flex-col md:flex-row md:items-end justify-between border-b border-white/10 pb-8 gap-8">
                <div>
                  <div className="flex items-center gap-3 mb-4">
                    <span className="w-2 h-2 rounded-full animate-pulse" style={{ backgroundColor: selectedGame.color }} />
                    <span className="text-xs font-bold tracking-[0.3em] uppercase" style={{ color: selectedGame.color }}>Video Archive</span>
                  </div>
                  <h1 className="text-5xl md:text-7xl font-black tracking-tighter text-white uppercase leading-none mb-2">
                    {selectedGame.title}
                  </h1>
                  <p className="text-[#888] font-light text-base md:text-lg max-w-xl">
                    High-definition broadcast records from {selectedGame.title}.
                  </p>
                </div>
                <div className="flex flex-col items-start md:items-end text-left md:text-right bg-white/[0.02] border border-white/5 p-4 md:p-6 min-w-[200px]">
                   <span className="text-[10px] font-bold text-[#888] uppercase tracking-widest mb-1">Total Runtime</span>
                   <span className="text-3xl font-mono text-white mb-4">4H 12M</span>
                   <button className="flex items-center gap-2 text-[10px] uppercase font-bold tracking-widest text-white hover:text-[#ff6a00] transition-colors">
                      <Play className="w-3 h-3" /> Play All
                   </button>
                </div>
              </div>
            </div>

            {/* Jaw-Dropped Masonry/Bento Grid for Videos */}
            <div className="px-4 md:px-8 xl:px-12 max-w-[1600px] mx-auto w-full">
              <div className="grid grid-cols-12 auto-rows-[250px] md:auto-rows-[350px] lg:auto-rows-[400px] gap-4 md:gap-6">
                {selectedGame.videos.map((video, index) => (
                  <motion.div 
                    key={index}
                    initial={{ opacity: 0, scale: 0.95 }}
                    whileInView={{ opacity: 1, scale: 1 }}
                    viewport={{ once: true, margin: "-50px" }}
                    transition={{ duration: 0.8, delay: (index % 3) * 0.1 }}
                    onHoverStart={() => setHoveredVideo(index)}
                    onHoverEnd={() => setHoveredVideo(null)}
                    className={`group cursor-pointer relative overflow-hidden bg-[#050505] border border-white/10 hover:border-white/30 transition-colors ${getVideoGridClass(index)}`}
                  >
                    <div className="absolute inset-0 bg-black/40 group-hover:bg-transparent transition-colors duration-700 z-10 pointer-events-none" />
                    
                    <img 
                      src={video.thumb} 
                      alt={video.title} 
                      className="w-full h-full object-cover transition-transform duration-[2s] ease-out group-hover:scale-[1.02] opacity-80 group-hover:opacity-100 filter contrast-[1.1]"
                    />

                    {/* Top Overlay metadata */}
                    <div className="absolute top-4 left-4 md:top-6 md:left-6 right-4 md:right-6 flex justify-between items-start z-20 transition-opacity duration-300">
                       <span className="px-3 py-1 bg-black/60 backdrop-blur-md border border-white/20 text-white text-[9px] font-bold uppercase tracking-widest">
                         {video.type}
                       </span>
                       <span className="px-3 py-1 bg-black/60 backdrop-blur-md border border-white/20 text-white text-[9px] font-mono font-bold tracking-widest">
                         {video.duration}
                       </span>
                    </div>

                    {/* Centered Play Button */}
                    <div className="absolute inset-0 z-20 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none">
                       <div className="w-16 h-16 md:w-20 md:h-20 rounded-full bg-[#ff6a00] flex items-center justify-center scale-75 group-hover:scale-100 transition-transform duration-500 shadow-[0_0_30px_rgba(255,106,0,0.5)]">
                          <Play className="w-6 h-6 md:w-8 md:h-8 text-black ml-1" fill="currentColor" />
                       </div>
                    </div>

                    {/* Bottom Title Overlay */}
                    <div className="absolute inset-x-0 bottom-0 p-4 md:p-6 bg-gradient-to-t from-black/90 via-black/40 to-transparent z-20 transform translate-y-4 group-hover:translate-y-0 transition-transform duration-500">
                        <div className="flex flex-col gap-1">
                           <h4 className="text-white font-bold text-lg md:text-xl uppercase tracking-tighter">{video.title}</h4>
                           <div className="flex items-center gap-3">
                              <span className="text-[#888] text-[10px] font-mono tracking-widest uppercase">{video.views} Views</span>
                              <div className="w-1 h-1 rounded-full bg-white/20" />
                              <span className="text-[#888] text-[10px] font-mono tracking-widest uppercase">4K HDR</span>
                           </div>
                        </div>
                    </div>
                  </motion.div>
                ))}
              </div>
            </div>
            
            {/* End of Gallery */}
            <div className="mt-24 text-center pb-12 w-full">
               <div className="w-px h-16 bg-gradient-to-b from-white/20 to-transparent mx-auto mb-6" />
               <p className="text-[#555] text-[10px] uppercase font-bold tracking-[0.3em]">End of Archive</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
  );
};
